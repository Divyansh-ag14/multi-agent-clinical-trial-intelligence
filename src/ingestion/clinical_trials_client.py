...
#   I tried httpx first: ClinicalTrials.gov returned 403 Forbidden.
...
# I am using requests instead of httpx because ClinicalTrials.gov
...
# I am  using it for API responses because the structure varies
...
# Instead of writing my own while loop with try/except and sleep(),
# tenacity gives clean decorators that handle all of that.
#
...
#                            I only use retry on network errors
...
# Every endpoint call is built from this base URL.
...
# note: ClinicalTrials.gov maximum is 1000
# If the API is slow or I hit a rate limit, smaller pages
# are easier to retry than large ones.

REQUEST_TIMEOUT = 30
# time in seconds to wait for the API to respond.
# If the API does not respond in 30 seconds we give up on that request.

MAX_RETRIES = 3
# Maximum number of times to retry a failed request.
# handles temporary network issues without crashing the pipeline.
...
# note: These headers are sent with every single API request.
...
# CLIENT CLASS
# I designed it as an "async context manager", meaning you
...
class ClinicalTrialsClient:
    ...
    def __init__(self):
        ...
        # started as None, it gets created in __aenter__.
    ...
    async def __aenter__(self) -> "ClinicalTrialsClient":
        ...
        # more efficient than creating a new connection
        # for every single API call we make.

        self._session.headers.update(HEADERS)
        # Apply the custom headers to every request this session makes.
    ...
    async def search_studies(
        ...
        # start empty and add to it with each page fetch.
        # At the end, this is what gets returned.

        next_page_token: str | None = None
        # ClinicalTrials.gov uses cursor based pagination.
        # After each page, the API returns a "nextPageToken" string.
        # send that token with the next request to get the next page.
        ...
        while len(all_studies) < max_results:
            ...
            # await: wait here until the API responds.
            ...
            if not next_page_token:
                # No next page token: the last page.
        ...
        # The last page might have pushed slightly over max_results.
    ...
    def _build_search_params(
        ...
        This method translates the Python arguments into
        the exact parameter names the API understands.

        Only includes parameters that were actually provided
        if condition is None, do not add query.cond to the request.
    ...
    @retry(
        ...
        # do NOT retry on HTTPError (404, 403) because those are
        # fault: retrying will not fix a wrong URL or parameter.
    )
    async def _fetch_page(self, params: dict[str, Any]) -> dict[str, Any] | None:
        ...
        def _get():
            ...
            # run it in a background thread via asyncio.to_thread().
            return self._session.get(
                f"{BASE_URL}/studies",
